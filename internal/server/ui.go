...
async function load(){var r=await fetch(A+'/routes').then(function(r){return r.json()});items=r.routes||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.pattern||'').toLowerCase().includes(q)||(i.target||'').toLowerCase().includes(q)||(i.method||'').toLowerCase().includes(q)});
...
if(i.pattern)h+='<div class="item-sub">'+esc(i.pattern)+'</div>';
...
if(i.priority)h+='<span>'+esc(i.priority)+'</span>';
if(i.rate_limit)h+='<span>'+esc(i.rate_limit)+'</span>';
...
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/routes/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",pattern:"",target:"",method:"",priority:"",rate_limit:"",enabled:"",hit_count:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' PATHFINDER</h2>';
h+='<div class="fr"><label>Name *</label><input id="f-name" value="'+esc(i.name)+'"></div>';
h+='<div class="fr"><label>Pattern</label><input id="f-pattern" value="'+esc(i.pattern)+'"></div>';
...
h+='<div class="fr"><label>Priority</label><input id="f-priority" value="'+esc(i.priority)+'"></div>';
h+='<div class="fr"><label>Rate Limit</label><input id="f-rate-limit" value="'+esc(i.rate_limit)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),pattern:document.getElementById('f-pattern').value.trim(),target:document.getElementById('f-target').value.trim(),method:document.getElementById('f-method').value.trim(),priority:document.getElementById('f-priority').value.trim(),rate_limit:document.getElementById('f-rate-limit').value.trim(),enabled:document.getElementById('f-enabled').value.trim(),hit_count:document.getElementById('f-hit-count').value.trim()};
if(editId){await fetch(A+'/routes/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/routes',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
